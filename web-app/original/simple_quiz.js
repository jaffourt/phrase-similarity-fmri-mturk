$(document).ready(function() {
	...
	myQuestions.forEach((currentQuestion, questionNumber) => {
		...
		for (i = 1; i <= 4; i++) { 
		    if (i==1) {
			q_choices[questionNumber].push (
			    `<div class="qa${questionNumber}" name="choice${ansNumber}" > MOST <input type="radio" name="answersT${ansNumber}" value="${i}" >`
			);
			counter+=1
			continue
		    }
		    if (i==4) {
			q_choices[questionNumber].push (
			    `<input type="radio" name="answersT${ansNumber}"  value="${i}"> LEAST </div>`
			);
			counter+=1
			break
		    }
	...
	if (validated == true){
		...
		for (var k = 0; k < 4; k++)
		{
		    line += i+1+","+transform_set[i][k]+","+answers[i][k]+"\n"
		}
	    }
